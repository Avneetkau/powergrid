...
<h3><strong>Use Email Id as Username And Employee-id as Password</strong></h3>
...
<li>
			...
			<span class="w3ls-text w3ls-name">
				<input type="password" name="empid" placeholder="Password" required=""/>
			</span>
...
<?php	
	...
	if(isset($_POST['login']))
	{
		...
		$pass=mysqli_real_escape_string ($con,$_POST['empid']);
		$rs2=mysqli_query($con,"SELECT * from `emp_entry` WHERE " .    
               "email='" . $user . "' AND " .           
               "empid='" . $pass . "'") or die(mysqli_error());
if(mysqli_num_rows($rs2)< 1)
        ...
        {
         $rs2=mysqli_query($con,"select * from emp_entry where email='$user' and empid='$pass'") or die(mysqli_error());
